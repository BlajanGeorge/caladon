...
- `GET …/cities/{id}` lists every unit type with `home`, `supporting` and `sentAway` (and `count`, kept
  ...
  in three tones. Hovering the numbers shows one tooltip for all three, a legend with a bullet per tone:
  "Own, in this city", "In this city, supporting from another city", "Away, supporting another city"
